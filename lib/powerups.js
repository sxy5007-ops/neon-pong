...
(function () {
  ...
  function applyPowerUp(owner, type) {
    ...
    if (type === 'freeze') applyTimedEffect(target, 'freeze', 1.5);
    if (type === 'multiball') {
    ...
    if (type === 'sound') {
      NP.AudioEngine.sfxSonicPulse();
      // Push all balls toward opponent's side
      var pushTo = owner === 'p1' ? NP.state.width - 80 : 80;
      NP.balls.forEach(function (ball) {
        var dir = owner === 'p1' ? 1 : -1;
        ball.x = Math.max(40, Math.min(NP.state.width - 40, pushTo + ball.r));
        ball.vx = Math.abs(ball.vx) * dir;
      });
      NP.spawnParticles(NP.ball.x, NP.ball.y, 40, ['#ff6b35', '#ff8c4a', '#f0e100']);
      NP.spawnParticles(NP.ball.x, NP.ball.y, 40, ['#ff6b35', '#ff8c4a', '#f0e100']);
      NP.state.shake = 14;
  ...
  function updatePowerUps(dt) {
    ...
    NP.powerUps.forEach(function (pu) {
      ...
      if (pu.y < Math.round(78 * s) || pu.y > NP.state.height - Math.round(78 * s)) pu.vy *= -1;
      pu.x = NP.clamp(pu.x, NP.state.width * 0.18, NP.state.width * 0.82);
      pu.y = NP.clamp(pu.y, Math.round(72 * s), NP.state.height - Math.round(72 * s));
    });
