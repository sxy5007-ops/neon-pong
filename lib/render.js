...
(function () {
  ...
  function render() {
    ...
    drawBumpers();
    // Events
    drawTornado();    // existing
    drawDisco();      // existing
    drawKaiju(ctx);   // new
    drawStorm(ctx);   // new
    drawTimePocket(ctx); // new
    drawVoidHole(ctx);   // new
    // Effects (in draw order after mains)
    drawPaddleHitBursts(ctx); // new
    drawNearMisses(ctx);      // new
    drawScoreFlash(ctx);      // new
    // Score flash overlay
    if (NP.state.flash > 0) {
      ctx.fillStyle = '#ffffff';
