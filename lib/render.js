...
(function () {
  ...
  function drawWindZones() {
    NP.WIND_ZONES.forEach(function (zone) {
  ...
  function drawOneBall(b) {
    ctx.save();
    ctx.shadowBlur = 24;
    ctx.shadowColor = NP.COLORS.ball;
    ctx.fillStyle = NP.COLORS.ball;
    ctx.beginPath();
  ...
  function drawPowerUpWarning(warning) {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(warning.x, warning.y, NP.POWER_UP_SIZE * 0.56 * s, 0, Math.PI * 2);
    ctx.stroke();
    ctx.beginPath();
  ...
  function render() {
    ...
    if (NP.paddles.p1 && NP.paddles.p2 && NP.ball) {
      ...
      NP.activeEffects.forEach(function (effect) {
    ...
    if (NP.state.flash > 0) {
      ...
      ctx.globalAlpha = NP.state.flash * 0.25;
  ...
  function addEffectPill(items, owner, effect, time) {
    ...
    el.className = 'effect-pill';
    el.textContent = owner + ' ' + NP.EFFECT_LABELS[effect] + ' ' + Math.ceil(time) + 's';
    items.appendChild(el);
  }
  NP.addEffectPill = addEffectPill;
