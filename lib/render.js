...
(function () {
  ...
  function drawLasers(ctx) {
    if (NP.state.lasers.length === 0) return;
    var s = NP.scale;
    var cfg = NP.config;
    ctx.save();
    NP.state.lasers.forEach(function (l) {
      var half = cfg.LASER_LENGTH * s * 0.5;
      ...
      ctx.lineWidth = cfg.LASER_WIDTH * s + 8;
      ctx.beginPath();
      ctx.moveTo(x1, y1);
      ctx.lineTo(x2, y2);
      ctx.stroke();
      ...
      ctx.lineWidth = cfg.LASER_WIDTH * s + 2;
      ctx.beginPath();
      ctx.moveTo(x1, y1);
      ctx.lineTo(x2, y2);
      ctx.stroke();
      ...
      ctx.lineWidth = cfg.LASER_WIDTH * s;
      ctx.beginPath();
      ctx.moveTo(x1, y1);
      ctx.lineTo(x2, y2);
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.arc(x1, y1, cfg.LASER_WIDTH * s + 3, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(x2, y2, cfg.LASER_WIDTH * s + 3, 0, Math.PI * 2);
      ctx.fill();
    });
    ctx.restore();
  }
  ...
  function drawWorm(ctx) {
    if (!NP.worm || !NP.worm.active) return;
    var segs = NP.worm.segments;
    var cfg = NP.config;
    var s = NP.scale;
    ctx.save();
    ...
    for (var i = 1; i < segs.length; i++) {
      ctx.shadowBlur = 15;
      ctx.shadowColor = cfg.WORM_COLOR;
      ctx.strokeStyle = 'rgba(57, 255, 20, 0.4)';
      ctx.lineWidth = cfg.WORM_SEGMENT_RADIUS * s * 0.5;
      ctx.beginPath();
    ...
    for (var j = segs.length - 1; j >= 0; j--) {
      var seg = segs[j];
      var r = cfg.WORM_SEGMENT_RADIUS * s;
      var isHead = (j === 0);
      if (isHead) r *= 1.4;
      var color = (j % 2 === 0) ? cfg.WORM_COLOR : '#00ff66';
      ctx.shadowBlur = isHead ? 30 : 18;
      ctx.shadowColor = cfg.WORM_COLOR;
      ctx.fillStyle = isHead ? '#00ff66' : color;
      ctx.beginPath();
      ...
      ctx.beginPath();
      ctx.arc(seg.x - r * 0.2, seg.y - r * 0.2, r * 0.4, 0, Math.PI * 2);
      ctx.fill();
    }
    // Eyes on head
    if (segs.length > 0) {
      ...
      ctx.fillStyle = '#ffffff';
      ctx.beginPath();
      ctx.arc(h.x - 4 * s, h.y - 3 * s, eyeR, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(h.x + 4 * s, h.y - 3 * s, eyeR, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.arc(h.x - 3 * s, h.y - 2 * s, eyeR * 0.5, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(h.x + 5 * s, h.y - 2 * s, eyeR * 0.5, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
  }
  ...
  NP.drawLasers = drawLasers;
  NP.drawWorm = drawWorm;
