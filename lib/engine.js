...
(function () {
  ...
  var CAMPAIGN_KEY = 'neon-pong-campaign-v1';
  var DEFAULT_CAMPAIGN_SAVE = { completedLevels: [], highestUnlocked: 0 };
  var DEFAULT_SAVE = {
  ...
  /* == Campaign Save == */
  NP.campaignSave = loadCampaignSave();

  function loadCampaignSave() {
    try {
      var raw = localStorage.getItem(CAMPAIGN_KEY);
      if (!raw) return { ...DEFAULT_CAMPAIGN_SAVE };
      return { ...DEFAULT_CAMPAIGN_SAVE, ...JSON.parse(raw) };
    } catch { return { ...DEFAULT_CAMPAIGN_SAVE }; }
  }

  function saveCampaignData() {
    try { localStorage.setItem(CAMPAIGN_KEY, JSON.stringify(NP.campaignSave)); } catch {}
  }

  function completeCampaignLevel(levelIndex) {
    if (NP.campaignSave.completedLevels.indexOf(levelIndex) === -1) {
      NP.campaignSave.completedLevels.push(levelIndex);
    }
    var next = levelIndex + 1;
    if (next > NP.campaignSave.highestUnlocked) {
      NP.campaignSave.highestUnlocked = next;
    }
    saveCampaignData();
  }
  ...
  NP.loadCampaignSave = loadCampaignSave;
  NP.saveCampaignData = saveCampaignData;
  NP.completeCampaignLevel = completeCampaignLevel;
