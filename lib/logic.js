...
(function () {
  ...
  if (!NP) return;
  /* == Campaign State == */
  NP.campaignLevel = null;
  NP.selectedCampaignLevel = -1;
  ...
  function updateOneBall(b, dt) {
    ...
    if (b.x + b.r < 0) {
      ...
      NP.state.shake = 2 + speedRatio * 2; NP.state.shakeDecay = 0.55;
      NP.state.flash = 0.35;
    ...
    if (NP.worm && NP.worm.active) {
      ...
      if (NP.worm.eatenBall) {
        ...
        if (NP.worm.eatenTimer <= 0) {
          ...
          NP.balls.push(NP.worm.eatenBall);
          NP.worm.eatenBall = null;
          NP.state.shake = 3;
          NP.state.shakeDecay = 0.55;
  ...
  function updateDisco(dt) {
    ...
    if (disco.active) {
      ...
      if (disco.cooldown <= 0) {
        disco.active = true;
        if (NP.AudioEngine.sfxCallout) NP.AudioEngine.sfxCallout('DIS-CO');
  ...
  function updateKaiju(dt) {
    ...
    if (!k.active) {
      ...
      if (k.cooldown <= 0) {
        k.warningTimer = 2.5;
        k.active = true;
        if (NP.AudioEngine.sfxCallout) NP.AudioEngine.sfxCallout('KAI-JU');
  ...
  function updateTimePocket(dt) {
    ...
    if (!tp.active) {
      ...
      if (tp.cooldown <= 0) {
        tp.active = true;
        if (NP.AudioEngine.sfxCallout) NP.AudioEngine.sfxCallout('TIME POCKET');
        tp.timer = 0;
  ...
  function renderCampaignGrid() {
    var grid = document.getElementById('campaign-level-grid');
    if (!grid) return;
    grid.innerHTML = '';
    var levels = NP.config.CAMPAIGN_LEVELS;
    for (var i = 0; i < levels.length; i++) {
      var lvl = levels[i];
      var btn = document.createElement('button');
      btn.className = 'campaign-level-btn';
      var isCompleted = NP.campaignSave.completedLevels.indexOf(i) !== -1;
      var isUnlocked = i <= NP.campaignSave.highestUnlocked;
      var isSelected = NP.selectedCampaignLevel === i;
      if (!isUnlocked) {
      ...
      num.className = 'campaign-level-num';
      num.textContent = (i + 1);
      btn.appendChild(num);
      var id = document.createElement('span');
      id.className = 'campaign-level-id';
      id.textContent = lvl.title;
      btn.appendChild(id);
      btn.addEventListener('click', function () {
        var idx = parseInt(this.getAttribute('data-level'), 10);
        if (idx > NP.campaignSave.highestUnlocked) return;
        NP.selectedCampaignLevel = idx;
        var info = document.getElementById('campaign-info');
        var title = document.getElementById('campaign-level-title');
        var desc = document.getElementById('campaign-level-desc');
        var detail = document.getElementById('campaign-level-detail');
        ...
        if (detail) {
          var parts = [];
          parts.push('WIN: ' + lvl.winScore + ' PTS');
          parts.push('DIFF: ' + lvl.difficulty.toUpperCase());
          if (lvl.initialSpeed) parts.push('SPEED: ' + lvl.initialSpeed);
          if (lvl.maxSpeed) parts.push('MAX: ' + lvl.maxSpeed);
  ...
  function showCampaignScreen() {
    ...
    showScreen('campaign');
    NP.state.screen = 'campaign';
  }

  function startCampaignLevel(levelIndex) {
    var lvl = NP.config.CAMPAIGN_LEVELS[levelIndex];
    if (!lvl) return;
    NP.campaignLevel = levelIndex;
    NP.selectedCampaignLevel = levelIndex;
    NP.startGame('campaign', lvl.difficulty, levelIndex);
  }

  function updateCampaignUI() {
    var badge = document.getElementById('campaign-complete-badge');
    if (!badge) return;
    var lastLevel = NP.config.CAMPAIGN_LEVELS.length - 1;
    if (NP.campaignLevel === lastLevel && NP.campaignSave.completedLevels.indexOf(lastLevel) !== -1) {
  ...
  function checkWin() {
    if (NP.scores.p1 >= NP.state.scoreLimit || NP.scores.p2 >= NP.state.scoreLimit) {
      ...
      if (NP.AudioEngine.sfxCallout) NP.AudioEngine.sfxCallout('VICTORY');
      // Win particle burst
      for (var i = 0; i < 4; i++) {
      ...
      if (NP.state.mode === 'campaign' && winnerKey === 'p1') {
        NP.completeCampaignLevel(NP.campaignLevel);
        var lastLevel = NP.config.CAMPAIGN_LEVELS.length - 1;
        if (NP.campaignLevel === lastLevel) {
          NP.ui.winnerText.textContent = 'CAMPAIGN COMPLETE!';
          var badge = document.getElementById('campaign-complete-badge');
  ...
  function startGame(mode, difficulty, campaignLevelIndex) {
    NP.AudioEngine.init();
    NP.AudioEngine.startMusic();
    ...
    if (campaignLevelIndex != null) {
      var lvl = NP.config.CAMPAIGN_LEVELS[campaignLevelIndex];
      if (lvl) {
        NP.state.scoreLimit = lvl.winScore;
      }
    }

    NP.ui.modeLabel.textContent = mode === 'ai' ? 'VS AI (' + difficulty.toUpperCase() + ')' : (mode === 'campaign' ? 'CAMPAIGN' : '2 PLAYERS');
  ...
  NP.quitToCampaign = quitToCampaign;
  NP.loop = loop;
  NP.renderCampaignGrid = renderCampaignGrid;
  NP.showCampaignScreen = showCampaignScreen;
  NP.startCampaignLevel = startCampaignLevel;
  NP.updateCampaignUI = updateCampaignUI;
